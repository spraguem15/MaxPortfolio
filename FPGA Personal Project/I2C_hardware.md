...
When it comes to verifying everything done here, I am verifying simple things first, such as correct state movement as well as values being correct. After I have verified that state movement is correct and my design is moving as expected, I am going to create a wrapper file that will work with my sensor. In other word the wrapper file will provide the correct values to the hardware so that signals can be sent over to the sensor and we can get actual values. Once I have these values I will convert them from raw data to proper temperature data (in fahrenheit) and will display them in binary on the FPGA dev board LEDs. The idea is here is that I can see whether or not my hardware is working before I start working with the I/O bus and the display screen. 
